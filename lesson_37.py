...
Exceptions are the problem which can occur at runtime and compile time.
Exception are divided into two categories such as
...
def test_one(x,y):
    try:
        ...
        #catch
        print(f'Something bad happened x:{x}, y:a{y}')
    finally:
        #Moving along
        print('Complete')
...
def test_two(x,y):
    try:
        ...
        #catch
        print(f'Something bad happened x:{x}, y:a{y}, issue: {e}')
    else:
        #trusted
        z = x / y
        print(f'Results: {z}')
